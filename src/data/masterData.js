...
const DEFAULTS = {
  ...
  salesStaffChannel: {},
  // (v1.0.438, ปรับปรุง v1.0.447, ขยายครอบคลุม v1.0.457) รายชื่อตำแหน่งงานมาตรฐาน — เดิมช่อง "ตำแหน่ง" ใน
  // หน้า User Management เป็นช่องพิมพ์เองล้วน (ไม่มี dropdown มาตรฐาน) ทำให้พิมพ์ไม่ตรงกันได้ (เช่น "เซลส์"/
  // "Sales"/"พนักงานขาย" ปนกัน) v1.0.438 เริ่มด้วยชุดตำแหน่งทั่วไปแบบเดา (รอข้อมูลจริง) — v1.0.447 แทนที่ด้วย
  // โครงสร้างตำแหน่งจริงของกลุ่มบริษัท เอส.เค. (34 ตำแหน่ง) ตามที่เจ้าของระบบให้รายละเอียดจริงมา — v1.0.457
  // ขยายเพิ่มตามคำขอ "ตำแหน่งงานหลากหลายครอบคลุมทั้งหมด ทั้งอดีต ปัจจุบัน อนาคต" คงตำแหน่งจริง 34 ตัวเดิมไว้
  // ทั้งหมด (ยืนยันจากผังองค์กรจริงแล้ว) แล้วเพิ่มตำแหน่งที่ธุรกิจกลุ่มรถยนต์/EV ระดับนี้ "อาจจะ" เคยมี/มีอยู่/
  // จะมีในอนาคต ครอบคลุมทุกฝ่ายให้กว้างที่สุดเผื่อขยายงานหรือมีคนมาสมัครตำแหน่งที่ยังไม่เคยมีคนจริงในระบบ —
  // เจ้าของระบบยังแก้ไข/เพิ่ม/ลบเองได้ทุกเมื่อที่หน้า Settings > Master Data (ใช้ MASTER_LISTS ด้านล่างเหมือน
  // list อื่นๆทุกจุด ไม่ใช่ hardcode ตายตัว) — หมายเหตุ: master_data doc มีอยู่แล้วใน Firestore production
  // ตั้งแต่ v1.0.438 ค่า DEFAULTS นี้จึงเป็นแค่ค่าเริ่มต้นสำหรับติดตั้งใหม่เท่านั้น ไม่ทับของเดิมที่มีอยู่แล้ว
  // อัตโนมัติ — "🔧 รีเซ็ต" ที่หน้า Master Data จะทับ "ทุก" ลิสต์ (สี/ธนาคาร/ฯลฯ) ไม่ใช่แค่ตำแหน่งงาน ห้ามใช้
  // เป็นวิธีอัปเดตแค่ตำแหน่งงาน ต้องเพิ่มทีละรายการผ่านปุ่ม "+ เพิ่ม" ที่หน้านั้น หรือให้ระบบกรอกให้ผ่านฟอร์ม
  // จริงถ้าล็อกอินอยู่เท่านั้น
  positions: [
    // ผู้บริหารระดับสูง/เจ้าของ
    'กรรมการผู้จัดการ/เจ้าของ', 'ผู้ช่วยเจ้าของกิจการ', 'รองกรรมการผู้จัดการ',
    'ผู้อำนวยการฝ่ายปฏิบัติการ (COO)', 'ผู้อำนวยการฝ่ายการเงิน (CFO)', 'เลขานุการผู้บริหาร',
    // ฝ่ายขายหน้าร้าน (แยกทีมตามแบรนด์ — DEEPAL/AION/OMODA&JAECOO)
    'ผู้จัดการทีมขายหน้าร้าน', 'พนักงานขายหน้าร้าน', 'ที่ปรึกษาการขายอาวุโส', 'เจ้าหน้าที่ธุรการฝ่ายขาย', 'ผู้ฝึกสอนทีมขาย (Sales Trainer)',
    // ฝ่ายขายออนไลน์ (ดูแลทุกบริษัทรวมกัน)
    'ผู้จัดการฝ่ายขายออนไลน์', 'ผู้จัดการทีมขายออนไลน์', 'พนักงานขายออนไลน์', 'แอดมินตอบแชท/ลูกค้าออนไลน์',
    // ฝ่ายขายเฉพาะทาง (เผื่อขยายธุรกิจในอนาคต)
    'เจ้าหน้าที่ขายรถองค์กร/ราชการ (Fleet Sales)', 'เจ้าหน้าที่ขายรถมือสอง', 'เจ้าหน้าที่รับซื้อ-แลกเปลี่ยนรถเก่า (Trade-in)', 'เจ้าหน้าที่ขายประกันภัยรถ',
    // ฝ่ายเซอร์วิส (ผู้จัดการฝ่ายคุมรวมทุกแบรนด์ ทีมงานแยกตามแบรนด์)
    'ผู้จัดการฝ่ายเซอร์วิส', 'ผู้จัดการ SA', 'เจ้าหน้าที่ลูกค้าสัมพันธ์', 'เจ้าหน้าที่ BP และประกันภัย', 'หัวหน้าช่าง', 'ช่างเทคนิค', 'ช่างเทคนิคอาวุโส',
    'ช่างสีและตัวถัง', 'ช่างทำความสะอาด/เคลือบสี (Detailing)', 'เจ้าหน้าที่รับรถ/แจ้งซ่อม (Service Reception)', 'เจ้าหน้าที่เคลมประกัน',
    'ช่างเทคนิคแบตเตอรี่ EV', 'ช่างเทคนิคสถานีชาร์จ', 'พนักงานขับรถยกลาก/ช่วยเหลือฉุกเฉิน',
    // แผนกแยกตามแบรนด์
    'เจ้าหน้าที่ธุรการ', 'เจ้าหน้าที่สต็อกรถ', 'เจ้าหน้าที่บัญชี', 'เจ้าหน้าที่คลังอะไหล่ประจำแบรนด์',
    // แผนกกลาง ดูแลทุกบริษัทในเครือ — บัญชี/การเงิน
    'หัวหน้าฝ่ายบัญชี', 'สมุห์บัญชี', 'เจ้าหน้าที่บัญชีอาวุโส', 'พนักงานบัญชีต้นทุน', 'เจ้าหน้าที่การเงิน', 'แคชเชียร์',
    'เจ้าหน้าที่ตรวจสอบภายใน (Internal Audit)', 'เจ้าหน้าที่ภาษี', 'เจ้าหน้าที่สินเชื่อ/ไฟแนนซ์รถ', 'เจ้าหน้าที่ติดตามหนี้/เร่งรัดหนี้สิน',
    // แผนกกลาง — บุคคล (HR)
    'เจ้าหน้าที่บุคคล (HR)', 'ผู้จัดการฝ่ายบุคคล (HR)', 'เจ้าหน้าที่สรรหาบุคลากร (Recruiter)',
    'เจ้าหน้าที่ฝึกอบรมและพัฒนา', 'เจ้าหน้าที่ค่าตอบแทนและสวัสดิการ', 'เจ้าหน้าที่แรงงานสัมพันธ์',
    // แผนกกลาง — PDI/ทะเบียน/ตรวจสอบคุณภาพ
    'เจ้าหน้าที่ PDI และแต่งรถ', 'เจ้าหน้าที่ทะเบียนและประกันภัยรถใหม่', 'เจ้าหน้าที่ตรวจสอบ',
    'เจ้าหน้าที่คุณภาพ (QA/QC)', 'เจ้าหน้าที่มาตรฐานการปฏิบัติงาน (SOP)',
    // แผนกกลาง — การตลาด/กราฟฟิก/ดิจิทัล
    'เจ้าหน้าที่การตลาด', 'ผู้จัดการฝ่ายการตลาด', 'นักออกแบบกราฟฟิก', 'ช่างภาพ/ตัดต่อวิดีโอ',
    'ผู้จัดการสื่อโซเชียล', 'นักเขียนคอนเทนต์', 'เจ้าหน้าที่ SEO/การตลาดดิจิทัล',
    // แผนกกลาง — ไอที/กฎหมาย/ลูกค้าสัมพันธ์/จัดซื้อ-โลจิสติกส์ (เผื่ออนาคต)
    'เจ้าหน้าที่ฝ่ายไอที', 'ผู้ดูแลระบบ (System Admin)', 'นักพัฒนาเว็บไซต์/แอปพลิเคชัน',
    'เจ้าหน้าที่กฎหมาย', 'เจ้าหน้าที่กำกับการปฏิบัติตามกฎ (Compliance)', 'เจ้าหน้าที่คุ้มครองข้อมูลส่วนบุคคล (PDPA)',
    'ผู้จัดการฝ่ายลูกค้าสัมพันธ์ (CX)', 'เจ้าหน้าที่คอลเซ็นเตอร์', 'เจ้าหน้าที่รับเรื่องร้องเรียน',
    'เจ้าหน้าที่จัดซื้อ', 'เจ้าหน้าที่โลจิสติกส์/ขนส่งรถ', 'พนักงานขับรถส่งมอบ', 'เจ้าหน้าที่คลังสินค้า',
    // เผื่อไว้ทั่วไป
    'เจ้าหน้าที่อะไหล่', 'หัวหน้าคลังอะไหล่', 'พนักงานต้อนรับ', 'รปภ.', 'แม่บ้าน', 'ช่างซ่อมบำรุงอาคาร',
    'พนักงานขับรถผู้บริหาร', 'ผู้จัดการสาขา', 'ผู้จัดการทั่วไป',
    // ระดับเริ่มต้น/ฝึกงาน (เผื่ออนาคต)
    'นักศึกษาฝึกงาน', 'พนักงานฝึกหัด (Management Trainee)',
  ],
}
