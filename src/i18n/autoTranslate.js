...
const TERMS = {
  ...
  // (v1.0.536) เพิ่มจากผลสแกน scripts/scan-i18n-gaps.mjs — หาข้อความไทยที่ซ้ำกันตรงเป๊ะข้ามอย่างน้อย 5 ไฟล์
  // ทั่วทั้ง src/pages ที่ยังไม่มีในดิกชันนารีนี้เลย "กำลังโหลด..." เจอซ้ำถึง 216 ไฟล์ (ข้อความ loading-state
  // มาตรฐานของทุกหน้า) เป็นจุดคุ้มค่าที่สุดจุดเดียวที่เพิ่มได้ — ส่วนที่เหลือเป็น label ฟอร์มที่มี " *" (บังคับ
  // กรอก) ต่อท้าย ซึ่งเป็นคนละ string เป๊ะๆกับ label เดิมที่มีอยู่แล้ว (ไม่มี *) จึงต้องเพิ่มแยกต่างหาก
  'กำลังโหลด...': ['Loading...', '加载中...'],
  'ไม่มีสิทธิ์เข้าถึงหน้านี้': ['No permission to access this page', '无权访问此页面'],
  'ชื่อลูกค้า *': ['Customer name *', '客户姓名 *'], 'ลูกค้า *': ['Customer *', '客户 *'],
  'รุ่นรถ *': ['Vehicle model *', '车型 *'], 'รุ่น *': ['Model *', '型号 *'],
  'ชื่อพนักงาน *': ['Staff name *', '员工姓名 *'], 'พนักงาน *': ['Staff *', '员工 *'],
  'ทะเบียนรถ *': ['License plate *', '车牌 *'], 'ทะเบียน *': ['License plate *', '车牌 *'],
  'หัวข้อ *': ['Topic *', '标题 *'], 'รายละเอียด *': ['Details *', '详情 *'],
  'สถานะ:': ['Status:', '状态:'], 'ระยะทาง (กม.)': ['Distance (km)', '距离 (公里)'],
  'ดูทั้งหมด →': ['View all →', '查看全部 →'], 'ราคา/หน่วย': ['Price/unit', '单价'],
}
