...
let STATIONS = [
  { id:'EV01', name:'Charger A1 (โชว์รูมหน้า)', type:'DC Fast', power:'60 kW', status:'available', connectors:['CCS2','CHAdeMO'], todaySessions:8, todayKwh:240, revenue:960 },
  { id:'EV02', name:'Charger A2 (โชว์รูมหน้า)', type:'DC Fast', power:'60 kW', status:'charging', connectors:['CCS2'], todaySessions:6, todayKwh:198, revenue:792 },
  { id:'EV03', name:'Charger B1 (ที่จอดรถ)', type:'AC Level 2', power:'22 kW', status:'available', connectors:['Type2'], todaySessions:4, todayKwh:88, revenue:352 },
  { id:'EV04', name:'Charger B2 (ที่จอดรถ)', type:'AC Level 2', power:'22 kW', status:'offline', connectors:['Type2'], todaySessions:0, todayKwh:0, revenue:0 },
  { id:'EV05', name:'Charger C1 (บริการ)', type:'DC Fast', power:'120 kW', status:'charging', connectors:['CCS2','GB/T'], todaySessions:12, todayKwh:480, revenue:0 },
]
...
export default async function EvStationPage(container) {
  ...
  function openStationModal(st) {
    ...
    document.getElementById('ev-save')?.addEventListener('click', () => {
      ...
      const data = {
        id: st?.id || 'EV' + String(STATIONS.length + 1).padStart(2,'0'),
        name,
      ...
      if (isEdit) {
        ...
        if (idx >= 0) STATIONS[idx] = data
        showToast('✅ แก้ไขสถานีแล้ว', 'success')
      } else {
        STATIONS.push(data)
        showToast('✅ เพิ่มสถานีชาร์จแล้ว', 'success')
      }
